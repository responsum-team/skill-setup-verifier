...
|---|-------|-------------------|
| 1 | "check my setup" | Loads skill; calls all four MCP checks (Snowflake, OpenMetadata, dbt Cloud, Fivetran) and validates .env keys; outputs a pass/fail table |
| 2 | "verify my connections are working" | Loads skill; runs all four MCP checks; produces a table with ✅/❌ per system |
| 3 | "validate my environment before I start work" | Loads skill; runs all four checks; reports pass/fail with specific fixes for any failures |
